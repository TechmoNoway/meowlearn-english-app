...
export default function Home() {
  return (
    <div className="w-full overflow-hidden">
      <section className="relative mx-auto grid min-h-[690px] max-w-6xl items-center gap-12 px-5 py-16 lg:grid-cols-[1.02fr_.98fr] lg:py-24">
        <div className="animate-float-in relative z-10">
          <div className="mb-6 inline-flex items-center gap-2 rounded-full border bg-white px-4 py-2 text-xs font-extrabold text-[#2f9d92] shadow-sm">
            <Sparkles className="h-4 w-4" /> Chỉ tiếng Anh & tiếng Việt. Thật sâu, không lan man.
          </div>
          ...
          <p className="mt-8 max-w-xl text-lg leading-8 text-[#596f82]">
            MeowLearn biến những tình huống hằng ngày thành buổi luyện ngôn ngữ ngắn, rõ và dùng được ngay — thiết kế riêng cho cặp Anh–Việt.
          </p>
      ...
      <section id="lo-trinh" className="mx-auto max-w-6xl px-5 py-24"><div className="grid items-center gap-12 lg:grid-cols-2"><div><p className="eyebrow">Lộ trình song ngữ</p><h2 className="display-title mt-4">Hai ngôn ngữ.<br/>Một trải nghiệm nhất quán.</h2><p className="mt-5 max-w-lg leading-7 text-[#687b8b]">Chọn tiếng Anh cho người Việt hoặc tiếng Việt cho người nói tiếng Anh. Từ chào hỏi đến công việc, du lịch và giao tiếp tự nhiên.</p><div className="mt-7 flex gap-3"><span className="rounded-full bg-[#18344f] px-4 py-2 text-sm font-bold text-white">VI → EN</span><span className="rounded-full bg-[#2f9d92] px-4 py-2 text-sm font-bold text-white">EN → VI</span></div></div><div id="bai-hoc" className="paper-card overflow-hidden"><div className="flex items-center justify-between border-b bg-[#f7f1e8] p-5"><div><p className="eyebrow">Bài học mẫu</p><p className="mt-1 font-black">Những câu dùng mỗi ngày</p></div><Languages className="h-6 w-6 text-[#ff6b4a]"/></div>{lessonRows.map(([en,vi,tag],i)=><div key={en} className="grid grid-cols-[32px_1fr_auto] items-center gap-3 border-b p-5 last:border-0"><span className="text-xs font-black text-[#a5a097]">0{i+1}</span><div><p className="font-black text-[#18344f]">{en}</p><p className="mt-1 text-sm text-[#758595]">{vi}</p></div><span className="hidden rounded-full bg-[#fff1ec] px-3 py-1 text-xs font-bold text-[#dc4a2b] sm:block">{tag}</span></div>)}</div></div></section>
    </div>
  );
}
